...
export const emailTemplates = {
  ...
  friendly: {
    ...
    body:
      ...
      "{{customerName}}" +
      `,</p>` +
      `<p>We hope this email finds you well! This is a friendly reminder that invoice #` +
      "{{invoiceNumber}}" +
      ` for $` +
      "{{amount}}" +
      ` is due on ` +
      ...
      `<p style="margin: 0;"><strong>Invoice Number:</strong> ` +
      "{{invoiceNumber}}" +
      `</p>` +
      `<p style="margin: 10px 0;"><strong>Amount Due:</strong> $` +
      "{{amount}}" +
      `</p>` +
  ...
  firm: {
    ...
    body:
      ...
      "{{customerName}}" +
      `,</p>` +
      `<p>Our records indicate that we have not yet received payment for invoice #` +
      "{{invoiceNumber}}" +
      ` in the amount of $` +
      "{{amount}}" +
      `, which was due on ` +
  ...
  urgent: {
    subject:
      "URGENT: Outstanding Payment Required for Invoice #" +
      "{{invoiceNumber}}",
    body:
      `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">` +
      ...
      "{{customerName}}" +
      `,</p>` +
      `<p><strong>This is an urgent notice regarding invoice #` +
      "{{invoiceNumber}}" +
      ` for $` +
      "{{amount}}" +
      ` which was due on ` +
      "{{dueDate}}" +
      ` and remains unpaid.</strong></p>` +
      ...
      `<p>Sincerely,<br/>Accounts Receivable<br/>` +
      "{{companyName}}" +
      `</p>` +
      `</div>`,
  },
