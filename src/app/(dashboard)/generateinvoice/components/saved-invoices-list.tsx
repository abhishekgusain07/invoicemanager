...
import { FolderOpen, Trash2, Calendar, DollarSign, FileText, Loader2 } from "lucide-react";
import { useState, useEffect } from "react";
import { toast } from "sonner";
import { loadGeneratedInvoices, deleteGeneratedInvoice } from "@/actions/generated-invoices";
...
interface SavedInvoicesListProps {
  onLoadInvoice: (invoiceData: InvoiceGenerationData, invoiceId: string) => void;
}
...
export function SavedInvoicesList({ onLoadInvoice }: SavedInvoicesListProps) {
  ...
  const handleLoadInvoice = (invoice: SavedInvoice) => {
    try {
      const invoiceData = JSON.parse(invoice.invoiceData) as InvoiceGenerationData;
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400">Save an invoice to see it here</p>
            </div>
          ) : (
              ...
              {savedInvoices.map((invoice) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 cursor-pointer" onClick={() => handleLoadInvoice(invoice)}>
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-semibold">{invoice.invoiceNumber}</h3>
                        {invoice.invoiceTitle && (
                          <span className="text-sm text-gray-600">• {invoice.invoiceTitle}</span>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>Due: {dayjs(invoice.paymentDue).format('MMM D, YYYY')}</span>
                        </div>
                        ...
                        <div className="text-xs text-gray-500">
                          Updated: {dayjs(invoice.updatedAt).format('MMM D, YYYY')}
                        </div>
                      </div>
                    </div>
