...
export const InvoicePaymentTotals = memo(function InvoicePaymentTotals({
  ...
  const vatSummary = invoiceData.items.reduce((acc, item) => {
  ...
  const totalNetAmount = invoiceData.items.reduce((sum, item) => sum + item.netAmount, 0);
  const totalVatAmount = invoiceData.items.reduce((sum, item) => sum + item.vatAmount, 0);

  return (
    <View style={[PDF_DEFAULT_TEMPLATE_STYLES.mb4]}>
      <View style={[PDF_DEFAULT_TEMPLATE_STYLES.flexRow]}>
        {/* VAT Summary Table (if visible) */}
        {invoiceData.vatTableSummaryIsVisible && Object.keys(vatSummary).length > 0 && (
          <View style={[{ width: "48%" }]}>
            ...
            <View style={[PDF_DEFAULT_TEMPLATE_STYLES.table]}>
              <View style={[PDF_DEFAULT_TEMPLATE_STYLES.tableRow, { backgroundColor: "#f5f5f5" }]}>
                ...
                <View style={[{ ...PDF_DEFAULT_TEMPLATE_STYLES.tableCol, width: "40%" }]}>
                  <Text style={[PDF_DEFAULT_TEMPLATE_STYLES.tableCell, PDF_DEFAULT_TEMPLATE_STYLES.boldText, PDF_DEFAULT_TEMPLATE_STYLES.smallText]}>
                    VAT Amount
                  </Text>
                </View>
              ...
              {Object.entries(vatSummary).map(([key, summary]) => (
                <View key={key} style={[PDF_DEFAULT_TEMPLATE_STYLES.tableRow]}>
                  <View style={[{ ...PDF_DEFAULT_TEMPLATE_STYLES.tableCol, width: "25%" }]}>
        ...
        <View style={[{ width: "48%" }]}>
          ...
          <View style={[PDF_DEFAULT_TEMPLATE_STYLES.flexRow, PDF_DEFAULT_TEMPLATE_STYLES.mb1]}>
            <Text style={[PDF_DEFAULT_TEMPLATE_STYLES.mediumText, { width: "60%" }]}>
              Net Total:
            </Text>
            <Text style={[PDF_DEFAULT_TEMPLATE_STYLES.mediumText, PDF_DEFAULT_TEMPLATE_STYLES.rightAlign, { width: "40%" }]}>
          ...
          <View style={[PDF_DEFAULT_TEMPLATE_STYLES.flexRow, PDF_DEFAULT_TEMPLATE_STYLES.borderT, PDF_DEFAULT_TEMPLATE_STYLES.py1]}>
            <Text style={[PDF_DEFAULT_TEMPLATE_STYLES.mediumText, PDF_DEFAULT_TEMPLATE_STYLES.boldText, { width: "60%" }]}>
              Total Amount:
            </Text>
            <Text style={[PDF_DEFAULT_TEMPLATE_STYLES.mediumText, PDF_DEFAULT_TEMPLATE_STYLES.boldText, PDF_DEFAULT_TEMPLATE_STYLES.rightAlign, { width: "40%" }]}>
              {formatCurrency(invoiceData.total)}
            </Text>
          </View>
          
          {/* Payment Method */}
          {invoiceData.paymentMethod && invoiceData.paymentMethodFieldIsVisible && (
