...
export default function InvoicesPage() {
  ...
  const openUpdateStatusModal = (invoiceId: string, currentStatus: string) => {
    // First, find the invoice to get its actual status
    const invoice = invoices.find(inv => inv.id === invoiceId);
    if (!invoice) {
      toast.error("Invoice not found");
      return;
    ...
    // Set the current invoice ID for updating
    setInvoiceToUpdate(invoiceId);
    
    // Set the current status from the actual invoice
  ...
  const handleUpdateStatusConfirm = async () => {
    ...
    try {
      // Make sure we're only updating if the status actually changed
      if (selectedStatus === currentInvoiceStatus) {
        toast.info("No changes made to invoice status");
      ...
      // Call the server action to update just this one invoice
      ...
      if (result.success) {
        ...
        // Update ONLY the main state array by ID
    ...
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <Dialog open={reminderModalOpen} onOpenChange={setReminderModalOpen}>
        <DialogContent 
          ...
          style={{ width: '60%', maxWidth: '90vw', height: 'fit-content', maxHeight: '80vh' }}
        >
          {selectedInvoice && (
            <>
