...
export function MyInvoicesClient() {
  ...
  // Fetch generated invoices using tRPC - optimized for prefetched data
  const {
    data: invoicesResult,
    isLoading,
    error,
  ...
  // Check if we're in a true loading state (no cached data available)
  const isTrueLoading = isLoading && !invoicesResult;
      ...
      <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={isTrueLoading}
            className="w-full md:w-auto"
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 ${isTrueLoading ? "animate-spin" : ""}`}
            />
            Refresh
          </Button>
