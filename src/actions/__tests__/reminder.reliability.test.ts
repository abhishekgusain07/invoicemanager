...
import { sendInvoiceReminder, getInvoiceReminderHistory, ReminderParams } from '../reminder';
...
describe('Email Sending Reliability Tests', () => {
  ...
  describe('Authentication Reliability', () => {
    it('should handle missing session gracefully', async () => {
      ...
      expect(result.error).toBe('Unauthorized. Please sign in to send reminders.');
  ...
  describe('Database Reliability', () => {
    it('should handle invoice not found', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe("Invoice not found or you don't have permission.");
    });
  ...
  describe('Token Management Reliability', () => {
    ...
    it('should handle missing refresh token', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Gmail account not connected. Please connect your Gmail account.');
    });
    ...
    it('should handle token service unavailable', async () => {
      mockGetUserRefreshToken.mockRejectedValue(new Error('Token service unavailable'));
  ...
  describe('Content Validation Reliability', () => {
    ...
    it('should handle HTML content properly', async () => {
      const htmlParams = {
        ...reminderParams,
        isHtml: true,
        emailContent: '<h1>Payment Reminder</h1><p>Please pay your invoice.</p>',
      };
    ...
    it('should provide meaningful error messages', async () => {
      const errorScenarios = [
        ...
        },
        {
          ...
          expectedError: 'Gmail account not connected. Please connect your Gmail account.',
        },
      ];
